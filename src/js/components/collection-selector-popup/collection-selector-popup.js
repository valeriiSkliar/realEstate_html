...
export const showCollectionSelectorPopup = (propertyId, propertyTitle) => {
  ...
  const collectionsWithProperty = getCollectionsWithProperty(propertyId).filter(collection => !collection.isFavorite);
                ...
                <div class="collection-selector-popup__item" data-collection-id="${collection.id}">
                  <div class="collection-selector-popup__item-checkbox">
                    <input type="checkbox" id="collection-${collection.id}" 
                      ${collectionsWithPropertyIds.includes(collection.id) ? 'checked' : ''}
                      ${collection.isFavorite && collectionsWithPropertyIds.includes(collection.id) ? 'disabled' : ''}
                    >
                  ...
                  <div class="collection-selector-popup__item-info">
                    <div class="collection-selector-popup__item-name">
                      ${collection.isFavorite ? '<i class="bi bi-star-fill"></i> ' : ''}${collection.name}
                    </div>
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', () => {
      if (isCreateMode) {
        const newName = newCollectionNameInput ? newCollectionNameInput.value.trim() : '';
        if (newName) {
          try {
  ...
  const collectionItems = popupContainer.querySelectorAll('.collection-selector-popup__item');
...
const saveCollectionSelections = (propertyId, propertyTitle) => {
  ...
  checkboxes.forEach(checkbox => {
    ...
    } else if (!shouldBeInCollection && wasInCollection && !collection.isFavorite) {
 ...
 */
export const addPropertyToFavorite = (propertyId, propertyTitle, showToast = true) => {
  ...
  if (!favoriteCollection) {
    ...
    if (favoriteCollection) {
      if (!favoriteCollection.isFavorite) {
        const updated = updateCollection(favoriteCollection.id, { isFavorite: true });
        if (updated) favoriteCollection.isFavorite = true;
        else console.warn(`Could not update 'Избранное' (ID: ${favoriteCollection.id}) to set isFavorite=true`);
      }
    } else {
      favoriteCollection = createCollection({ name: 'Избранное', notes: 'Автоматически созданная подборка для избранных объектов' });
      if (favoriteCollection && favoriteCollection.id) {
        const updated = updateCollection(favoriteCollection.id, { isFavorite: true });
         if (updated) favoriteCollection.isFavorite = true;
         else console.warn(`Could not update newly created 'Избранное' (ID: ${favoriteCollection.id}) to set isFavorite=true`);
      } else {
        console.error("Failed to create 'Избранное' collection.");
        return { action: 'error', success: false, isFavorite: false, message: 'Failed to create favorite collection.' };
      }
    }
  }
  ...
  if (!favoriteCollection || !favoriteCollection.id) {
    ...
    return { action: 'error', success: false, isFavorite: false, message: 'Favorite collection ID missing.' };
  }

  const isAlreadyFavorite = isPropertyInCollection(favoriteCollection.id, propertyId);
  ...
  if (isAlreadyFavorite) {
    ...
    if (added) {
      ...
      console.warn(`Failed to add property ${propertyId} to 'Избранное' (ID: ${favoriteCollection.id})`);
